...
mean_cube = np.load('deepmotion/c3d/models/train01_16_128_171_mean.npy')
...
def get_temporal_model(layers=[100]):
    ...
    input_features = Input(batch_shape=(1, 4096,), name='features')
    ...
    temp_model = Model(input=input_features, output=output)
...
def main():
    ...
    for _ in range(epochs):
        for X, Y in zip(x_train, y_train):
            ...
            temporal_model.train_on_batch(np.array(X), Y)
