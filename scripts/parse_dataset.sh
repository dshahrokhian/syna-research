...
function landmark_video {
  ../syna/OpenFace/build/bin/FaceLandmarkVid -q -f "$1" -ov "$2" >> parse_dataset.log
}

# Extract features using OpenFace
function feature_extraction {
  ../syna/OpenFace/build/bin/FeatureExtraction -q -rigid -wild -multi-view 1 -f "$1" -of "$2" >> parse_dataset.log
}
